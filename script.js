...
const callTypeHint = document.getElementById('callTypeHint');
const callDirectionHint = document.getElementById('callDirectionHint');
...
function buildCallTypeUI(){
  ...
  callTypeHint.textContent = supported
    ? (callType==='external' ? 'A Carrier / SIP Trunk leg is inserted between the PBX and the far end.' : 'Both parties are treated as being on the same local system.')
    : 'Not applicable to this simulation — no PBX↔far-end leg to route externally.';
}
function buildCallDirectionUI(){
  ...
  callDirectionHint.textContent = supported
    ? (callDirection==='inbound' ? 'User B originates the call; User A is the one being reached.' : 'User A originates the call; User B is the one being reached.')
    : 'Not applicable to this simulation — there’s no second party to swap.';
}
const TRANSPORT_HINTS = {
  udp: 'UDP: connectionless — the sender must retransmit if no response arrives in time. Port 5060.',
  tcp: 'TCP: connection-oriented and reliable — no SIP-level retransmission needed, but a handshake happens first. Port 5060.',
  tls: 'TLS (SIPS): encrypted end-to-end — needs a TLS handshake before the first message. Port 5061.'
};
function buildTransportUI(){
  ...
  transportHint.textContent = TRANSPORT_HINTS[transportMode] || '';
}
...
function resetState(){
  ...
  svg.innerHTML = '';
  currentGlowEl = null;
  const diagramEmptyState = document.getElementById('diagramEmptyState');
  if(diagramEmptyState) diagramEmptyState.style.display = 'flex';
  logBody.innerHTML = '';
