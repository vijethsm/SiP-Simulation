...
(function landingPageInit(){
  ...
  const BOOT_SEQUENCE = [
    ...
    '> Loading 63 scenarios across 15 categories...',
    '> Establishing signaling engine (SIP / SDP / RTP)...',
    '> Loading fault injection modules...',
    '> Ready.'
  ];
  ...
  function runBootSequence(){
    ...
    BOOT_SEQUENCE.forEach((text, i)=>{
      ...
      el.innerHTML = isLast
        ? '<span class="boot-ok">'+text+'</span><span class="boot-cursor"></span>'
        : text;
      bootLines.appendChild(el);
      setTimeout(()=>{ el.classList.add('show'); }, delay + i*perLine);
    });
    setTimeout(finishBoot, delay + BOOT_SEQUENCE.length*perLine + 500);
  }
